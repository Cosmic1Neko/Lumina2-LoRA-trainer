...
$pretrained_model = "/root/autodl-tmp/model/neta-lumina-alpha-raw-ep7-s160000.safetensors" # base model path | 底模路径
$vae = "/root/autodl-tmp/model/vae/Pad-Flux-EQ-v2-B1.safetensors" # 使用Flux EQ-VAE版本 (Anzhc/MS-LC-EQ-D-VR_VAE/Pad Flux EQ v2 B1.safetensors)
...
# Adopt
# ProdigyPlusScheduleFree, RAdamScheduleFree
$d_coef = "2.0" #prodigy D上升速度
...
elseif ($train_mode -ieq "sd3_lora") {
  ...
  if ($split_mode -ne 0 -or $split_qkv -ne 0 -or $train_t5xxl -ne 0 -or $enable_sd3_block -ne 0 -or $enable_sd3_block_dim -ne 0) {
    [void]$ext_args.Add("--network_args")
    if ($split_qkv -ne 0 -and $enable_lycoris -ne 1) {
      [void]$ext_args.Add("split_qkv=True")
    }
    ...
    if ($enable_sd3_block_dim -ne 0) {
      [void]$ext_args.Add("context_attn_dim=$context_attn_dim")
      [void]$ext_args.Add("context_mlp_dim=$context_mlp_dim")
      [void]$ext_args.Add("context_mod_dim=$context_mod_dim")
      [void]$ext_args.Add("x_attn_dim=$x_attn_dim")
      [void]$ext_args.Add("x_mlp_dim=$x_mlp_dim")
      [void]$ext_args.Add("x_mod_dim=$x_mod_dim")
      [void]$ext_args.Add("emb_dims=$emb_dims")
    }
    if ($enable_sd3_block -ne 0) {
      [void]$ext_args.Add("train_block_indices=$train_block_indices")
    }
    # if ($split_mode -ne 0 -and $enable_lycoris -ne 1) {
    #   [void]$ext_args.Add("train_blocks=single")
    #   [void]$ext_args.Add("--split_mode")
...
elseif ($train_mode -ieq "lumina_lora") {
  $network_module = $network_module + "_lumina"
}
...
if ($train_mode -ilike "*cn3l" -or $train_mode -ilike "*controlnet") {
  if ($train_mode -ilike "*controlnet") {
    $laungh_script = "train_control_net"
    if ($controlnet_model_name_or_path) {
      [void]$ext_args.Add("--controlnet_model_name_or_path=$controlnet_model_name_or_path")
    }
    if ($train_mode -ieq "sdxl_controlnet") {
      if ($control_net_lr) {
        [void]$ext_args.Add("--control_net_lr=$control_net_lr")
      }
    }
  }
  else {
...
if ($train_mode -ilike "lumina*") {
  ...
  if ($use_flash_attn) {
    [void]$ext_args.Add("--use_flash_attn")
  }
  if ($cfg_trunc -ne 0.25) {
    [void]$ext_args.Add("--cfg_trunc=$cfg_trunc")
  }
  if ($renorm_cfg -ne 1.0) {
    [void]$ext_args.Add("--renorm_cfg=$renorm_cfg")
  }
  if ($system_prompt) {
    [void]$ext_args.Add("--system_prompt=$system_prompt")
  }
  if ($cache_text_encoder_outputs -ne 0) { 
    [void]$ext_args.Add("--cache_text_encoder_outputs")
    if ($cache_text_encoder_outputs_to_disk -ne 0) { 
...
if ($train_mode -ilike "sd3*" -or $train_mode -ilike "flux*") {
  ...
  if ($train_mode -ilike "flux*") {
    ...
    if ($sigma_max_scale -ne 1.0) {
      [void]$ext_args.Add("--sigma_max_scale=$sigma_max_scale")
    }
    if ($training_shift -ne 1.0) {
      [void]$ext_args.Add("--training_shift=$training_shift")
    }
    if ($resolution_shift) {
      [void]$ext_args.Add("--resolution_shift")
    }
  }
  if ($cache_text_encoder_outputs -ne 0) { 
    [void]$ext_args.Add("--cache_text_encoder_outputs")
  ...
  if ($weighting_scheme -ine "uniform") {
...
if ($optimizer_type -ilike "*ScheduleFree") {
  ...
  if ($optimizer_type -ieq "ProdigyPlusScheduleFree") {
    $lr = "1"
    if ($unet_lr) {
      $unet_lr = $lr
    }
    ...
    [void]$ext_args.Add("--optimizer_type=prodigyplus.ProdigyPlusScheduleFree")
    [void]$ext_args.Add("--optimizer_args")
    #[void]$ext_args.Add("weight_decay=0.08")
    [void]$ext_args.Add("use_bias_correction=True")
    [void]$ext_args.Add("d_coef=$d_coef")
    #[void]$ext_args.Add("use_cautious=True")
    #[void]$ext_args.Add("use_adopt=True")
    if ($d0) {
      [void]$ext_args.Add("d0=$d0")
    }
    if ($prodigy_steps -ne 0) {
      [void]$ext_args.Add("prodigy_steps=$prodigy_steps")
    }
  }
  else {
    $lr_scheduler = ""
...
if ($optimizer_type -ieq "DeMo") {
  [void]$ext_args.Add("--optimizer_type=pytorch_optimizer.DeMo")
}
...
if ($loss_type -ne "l2") {
  ...
  if ($huber_scale -ne 1.0) {
    [void]$ext_args.Add("--huber_scale=$huber_scale")
  }
  if ($huber_c -ne 0.1) {
